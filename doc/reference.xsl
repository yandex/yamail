...
<!--
  Copyright (c) 2008-2014 Nikki Chumakov (nikki at yandex-team dot ru)
...
[include requirements/SimpleQueue.qbk]
[include requirements/CancellableQueue.qbk]
<!--
  ...
  <xsl:for-each select="
    ...
    <xsl:choose>
      <xsl:when test="@kind='class' or @kind='struct'">
        <xsl:if test="
            contains(compoundname, 'yamail::') and
...
<xsl:template name="strip-yamail-ns">
  <xsl:param name="name"/>
  <xsl:choose>
    <xsl:when test="contains($name, 'boost::system::is_error_code_enum')">
      <xsl:value-of select="$name"/>
    </xsl:when>
    <xsl:when test="contains($name, 'yamail::')">
      <xsl:value-of select="substring-after($name, 'yamail::')"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="$name"/>
    </xsl:otherwise>
...
<xsl:template name="make-id">
  ...
  <xsl:choose>
    ...
    <xsl:when test="contains($name, 'yamail::error::')">
      <xsl:call-template name="make-id">
        <xsl:with-param name="name"
         select="concat(substring-before($name, 'yamail::error::'), substring-after($name, 'yamail::error::'))"/>
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="contains($name, '::')">
      <xsl:call-template name="make-id">
...
<xsl:template match="ref[@kindref='compound']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'yamail') or contains($name, 'yamail::')">
      <xsl:variable name="dox-ref-id" select="@refid"/>
      <xsl:variable name="ref-name">
        <xsl:call-template name="strip-yamail-ns">
          <xsl:with-param name="name"
      ...
      <xsl:text>[link yamail.reference.</xsl:text>
...
<xsl:template match="ref[@kindref='member']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'namespaceboost_1_1yamail') and count($memberdefs) &gt; 0">
...
<xsl:template name="header-requirements">
  ...
  <xsl:text>[^yamail/</xsl:text>
  <xsl:value-of select="substring-after($file, 'yamail/')"/>
  <xsl:text>]</xsl:text>
  <xsl:value-of select="$newline"/>
  <xsl:value-of select="$newline"/>
  <xsl:text>[*Convenience header: ]</xsl:text>
  <xsl:choose>
    <xsl:when test="contains($file, 'yamail/active')">
      <xsl:text>[^yamail/active.h]</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'yamail/ssl')">
      <xsl:text>[^yamail/ssl.h]</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'yamail/high_resolution_timer')">
      <xsl:text>None</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'yamail/steady_timer')">
      <xsl:text>None</xsl:text>
    </xsl:when>
    <xsl:when test="contains($file, 'yamail/system_timer')">
      <xsl:text>None</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>[^yamail.h]</xsl:text>
    </xsl:otherwise>
  </xsl:choose>
  <xsl:value-of select="$newline"/>
</xsl:template>
...
<xsl:text>    </xsl:text><xsl:if test="@prot='public'">public </xsl:if><xsl:call-template
 name="strip-yamail-ns"><xsl:with-param name="name" select="."/></xsl:call-template><xsl:if
...
<xsl:choose>
  <xsl:when test="count(name) &gt; 0">
    [[link yamail.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of select="name"/>
  ...
  <xsl:otherwise>
    ...
    [[link yamail.reference.<xsl:value-of select="$type-id"/>
...
<xsl:for-each select="sectiondef[@kind='public-func' or @kind='public-static-func']/memberdef" mode="class-table">
  ...
  [
    [[link yamail.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of select="$id"/>
...
</xsl:if>``[link yamail.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of
...
['Inherited from <xsl:call-template name="strip-yamail-ns">
...
<xsl:text>
      ...
      <xsl:choose>
        ...
        <xsl:when test="declname = 'SocketService1' or declname = 'SocketService2'">
          <xsl:value-of select="concat('``[link yamail.reference.SocketService ', declname, ']``')"/>
        </xsl:when>
        ...
        <xsl:otherwise>
          <xsl:value-of select="concat('``[link yamail.reference.', declname, ' ', declname, ']``')"/>
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:text>  </xsl:text><xsl:if test="string-length($stripped-type) &gt; 0"><xsl:value-of
 select="$stripped-type"/><xsl:text> </xsl:text></xsl:if>``[link yamail.reference.<xsl:value-of
